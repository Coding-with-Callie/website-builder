...
func Connect() {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to connect to default postgres database")
	}
	...
	if err != nil {
		if err.Error() == "pq: database \"website_builder\" already exists" {
			log.Info().Msg("Website builder database already exists")
		} else {
			log.Fatal().Err(err).Msg("Failed to create website builder database")
		}
	}
	...
	err = db.Close()
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to close connection to default database")
	}
	...
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to connect to website builder database")
	}

	// Check if the connection is successful
	err = db.Ping()
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to ping website builder database")
	}

	log.Info().Msg("Connected to website builder database")
